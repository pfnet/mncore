...
while getopts "Ai:s:R:O:h" opt; do
    case "${opt}" in
    A) USE_ALL=1;;
    i) IMAGE=${OPTARG};;  # localhost/ prefix is required when run with -R podman, that is, the options would be -R podman -i localhost/mncore-sdk-full:0.5
